...
function architectureLayout(model) {
  ...
  const accessDefs = [
    ...
    ...(model.hasOnPrem ? [{ title: "On-Prem ↔ Cloud", sub: cloudMeta.vpn, iconKind: "vpn" }] : []),
    { title: "Corporate DNS", sub: "Resolves *.pe.jfrog.io", iconKind: "dns" },
  ...
  const peBoxW = 210;
  const routeGapX = 16;
  const routeBandW = Math.max(accessRowW, peBoxW + routeGapX + 500);
  ...
  // Optional path for clients on a private/on-prem network: they don't have to go through
  // the VPN + private-endpoint chain at all — they can reach the public DNS-routing URL
  // directly over the internet instead (no private connectivity set up). Drawn only when
  // an On-Prem ↔ Cloud link exists, since that's what makes the VPN path "the alternative"
  // rather than the only option.
  const clientsNode = accessNodes[0];
  const clientBypass = model.hasOnPrem ? {
    x: clientsNode.x + clientsNode.w / 2,
    fromY: accessBoxBottom,
    toY: routeBoxY,
    label: "optional: direct, no VPN",
  } : null;
  ...
  const route = {
    ...
    sub: `Manual failover / geo-location · ${primaries.length + additionals.length} writable site(s) · does not cover Private Endpoints (left) — those are pinned per JPD`,
...
function buildArchitectureSvg(model) {
  ...
  acc.connectors.forEach((c) => {
    connectors.push(`<line x1="${c.x1}" y1="${c.y1}" x2="${c.x2}" y2="${c.y2}" stroke="${C.net}" stroke-width="1.4" marker-end="url(#geo-net)"/>`);
  });
  ...
  connectors.push(`<path d="M ${route.inFromX} ${route.inFromY} C ${route.inFromX} ${(route.inFromY + route.inToY) / 2}, ${route.inToX} ${(route.inFromY + route.inToY) / 2}, ${route.inToX} ${route.inToY}" fill="none" stroke="${C.net}" stroke-width="1.4" marker-end="url(#geo-net)"/>`);
  ...
  if (route.clientBypass) {
    ...
    connectors.push(`<path d="M ${cb.x} ${cb.fromY} C ${cb.x} ${(cb.fromY + cb.toY) / 2}, ${cb.x} ${(cb.fromY + cb.toY) / 2}, ${cb.x} ${cb.toY}" fill="none" stroke="${C.muted}" stroke-width="1.2" stroke-dasharray="3 4" marker-end="url(#geo-net)"/>`);
  ...
  const routeFlow = `stroke="${C.route}" stroke-width="1.6" stroke-dasharray="7 5" marker-end="url(#geo-route)"`;
  ...
  if (route.additionalFan) {
    ...
    connectors.push(`<path d="M ${route.outX} ${route.outFromY} L ${fan.sideX} ${route.outFromY} L ${fan.sideX} ${fan.busY}" fill="none" stroke="${C.route}" stroke-width="1.6" stroke-dasharray="7 5"/>`);
    const fanXs = fan.targets.map((t) => t.x);
    connectors.push(`<line x1="${fan.sideX}" y1="${fan.busY}" x2="${Math.min(fan.sideX, ...fanXs)}" y2="${fan.busY}" stroke="${C.route}" stroke-width="1.6" stroke-dasharray="7 5"/>`);
  ...
  if (pf) {
    connectors.push(`<path d="M ${pf.inFromX} ${pf.inFromY} C ${pf.inFromX} ${(pf.inFromY + pf.inToY) / 2}, ${pf.inToX} ${(pf.inFromY + pf.inToY) / 2}, ${pf.inToX} ${pf.inToY}" fill="none" stroke="${C.net}" stroke-width="1.4" marker-end="url(#geo-net)"/>`);
    ...
    const peDash = `stroke="${C.net}" stroke-width="1.4" stroke-dasharray="4 3"`;
...
function buildArchitectureDrawio(model) {
  ...
  if (route.clientBypass) {
    cells.push(connect("optional: direct, no VPN", `html=1;dashed=1;dashPattern=3 4;endArrow=blockThin;endFill=1;strokeColor=${C.muted};strokeWidth=1.2;fontColor=${C.muted};fontSize=9;edgeStyle=orthogonalEdgeStyle;exitX=0.5;exitY=1;entryX=0.15;entryY=0;`, acc.nodes[0].id, route.id));
  }
