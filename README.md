...
- **Architecture diagram** — regions with Primary JPDs, Additional Platform Instances, Edges, and **animated** federation / distribution links, plus a client/network-access overview band: internal clients, an On-Prem ↔ Cloud VPN/Direct Connect link, corporate DNS with a **conditional DNS resolver hop**, and a load balancer — every arrow in the whole diagram animates (not just federation/distribution), and every resource gets a **real icon**: AWS/Azure/GCP service icons on the cloud-side boxes, and JFrog's own logo on every Primary/Additional/Edge box and the routing box, all embedded as self-contained images (not references to draw.io's internal shape-library names — see the `.drawio`/Lucid note below for why). **Private Endpoint(s)** sit in their own box next to the **JFrog-side DNS routing (public)** box — both fan directly into every writable site, but never into each other: per [DNS Routing in MyJFrog](https://docs.jfrog.com/administration/docs/dns-routing-in-myjfrog), the public routing URL's failover/geo-routing does not extend to PrivateLink/Private Link/PSC connections, which are pinned one-per-JPD, so the diagram wires them as two separate, parallel paths. Rounding it out: a SaaS/self-managed auth callout and a monitoring callout tailored to the deployment model and cloud provider(s).

  **On-prem CI/CD clients are always shown, regardless of what was purchased.** On-prem CI/CD reaching a cloud-hosted JFrog instance is a scenario every customer has, whether or not their order includes an actual On-Prem JPS region — so the whole client-connectivity picture is drawn unconditionally, with three paths off "Internal clients": (1) **via VPN** — On-Prem ↔ Cloud → Corporate DNS (*conditional forwarder* for the `pe.jfrog.io` zone) → **DNS Resolver Endpoint** (a cloud-side inbound resolver in the VPC/VNet — Route 53 Resolver inbound endpoint on AWS, Azure DNS Private Resolver inbound endpoint on Azure, Cloud DNS inbound server policy on GCP) → Load Balancer → Private Endpoint; (2) **cloud-native** — a client/service already inside the same VPC/VNet skips the VPN and DNS bridge entirely and reaches the Private Endpoint directly; (3) **public** — no private connectivity at all, straight to the public routing URL via JFrog-side DNS. The On-Prem JPS/JPD box itself in the writable-site grid still only appears when the order actually includes one. The **PS kickoff checklist** spells out the exact private-path setup: create the inbound resolver endpoint, note its IP(s), then add a conditional forwarder on the on-prem DNS server (Windows DNS, BIND, Infoblox, etc.) for that zone pointing at those IPs, and validate with `nslookup` before cutover.
...
The `.xml` file is an uncompressed draw.io document built from the same layout pass as the SVG: lanes, role labels, node boxes, and real source→target connectors (never a bare source/target-less "point" edge). It opens directly and reliably in draw.io as native, editable shapes. Cloud-provider icons (VPN, DNS, DNS resolver, load balancer, private endpoint) are embedded as self-contained `image=data:image/svg+xml;base64,...` shapes — the exact same vector/image data used in the live SVG preview — rather than references to draw.io's internal shape-library names (e.g. `shape=mxgraph.gcp2.*`). An earlier version used those internal names for GCP, guessed by naming convention and never actually confirmed against draw.io's registry; they rendered blank for users. Embedding the icon directly has no such dependency and can't go stale across draw.io versions.
